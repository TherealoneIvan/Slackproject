...
{
public:
    typedef AbstractMetaObject<Base, Args...> Meta;
    typedef Manifest<Base, Args...> Manif;
    ...
    Iterator loadLibrary(const std::string& path, const std::string& manifest)
        ...
        /// for the library to become unloaded.
    {
        ...
        if (it == _map.end())
        {
            ...
            try
            {
                li.pLibrary  = new SharedLibrary(path, SharedLibrary::SHLIB_LOCAL);
                ...
                if (li.pLibrary->hasSymbol(pocoBuildManifestSymbol))
                {
                    ...
                    if (buildManifest(const_cast<Manif*>(li.pManifest))) {
                        return Iterator(_map.emplace(path, li).first);
                    }
                    else
    ...
    Iterator loadLibrary(const std::string& path)
    ...
    Base* create(const std::string& className, Args... args) const
        /// Creates an instance of the given class.
        /// Throws a NotFoundException if the class
        /// is not known.
    {
        return classFor(className).create(args...);
    }
